let gameBoard = [
  ...
  function insertarBarco(tablero, barco) {
    ...
    for (let i = posbarco; i < posbarco + barco.length; i++) {
      if (
        // Para la fila 1 (0 del array)
        fila == 0 &&
        (tablero[fila][i] == 1 || //Comprueba si hay barco en la posición elegida
          tablero[fila][posbarco - 1] == 1 || // Comprueba si hay barco en la casilla anterior a la primera del barco
          tablero[fila][posbarco + barco.length] == 1 || // Comprueba si hay barco en la casilla después a la del barco
          tablero[fila + 1][i] == 1) //Comprueba si hay barco en la fila de debajo de la posición del barco
      ) {
        contador = contador + 1;
      } else if (
        //Para la fila 10 (9 del arary)
        fila == tablero.length - 1 &&
        (tablero[fila][i] == 1 || //Comprueba si hay barco en la posición elegida
          tablero[fila][posbarco - 1] == 1 || // Comprueba si hay barco en la casilla anterior a la primera del barco
          tablero[fila][posbarco + barco.length] == 1 || // Comprueba si hay barco en la casilla después a la del barco
          tablero[fila - 1][i] == 1) //Comprueba si hay barco en la fila de superior de la posición del barco
      ) {
        contador = contador + 1;
      } else if (
        //Para las filas 2 a 9 ( 1 a 8 del array)
        fila > 0 &&
        fila < tablero.length - 2 &&
        (tablero[fila][i] == 1 || //Comprueba si hay barco en la posición elegida
          tablero[fila][posbarco - 1] == 1 || // Comprueba si hay barco en la casilla anterior a la primera del barco
          tablero[fila][posbarco + barco.length] == 1 || // Comprueba si hay barco en la casilla después a la del barco
          tablero[fila - 1][i] == 1 || //Comprueba si hay barco en la fila de superior de la posición del barco
          tablero[fila + 1][i] == 1) //Comprueba si hay barco en la fila inferior a la posición del barco
      ) {
        contador = contador + 1;
      }
    }
